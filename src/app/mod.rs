...
impl Application {
    ...
    pub fn clear_mouse_pos(&mut self) {
        let mut guard = self.mouse_pos.write();
        *guard = None;
    }

    pub fn update_mouse_pos(&mut self, new_pos: LogicalPosition<f32>) {
        let mut guard = self.mouse_pos.write();
        *guard = Some(new_pos);
    }

    pub fn get_mouse_pos(&self) -> Option<LogicalPosition<f32>> {
        let a = self.mouse_pos.read();
        *a
    }
